...
model = WhisperModel(model_size_or_path=WHISPER_MODEL, device=WHISPER_DEVICE, compute_type=WHISPER_COMPUTE)
...
def lang_cb(call):
    ...
    try:
        text = whisper_transcribe(path, language=code)
        if not text:
...
def handle_media(message):
    ...
    file_path = os.path.join(DOWNLOADS_DIR, f"{message.id}_{getattr(media, 'file_unique_id', '')}")
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = whisper_transcribe(file_path, language=lang)
        if not text:
