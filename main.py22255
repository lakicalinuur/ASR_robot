...
def transcribe_via_url_assemblyai(audio_url, language=None):
...
def lang_cb(call):
    ...
    try:
        text = transcribe_via_url_assemblyai(file_url, language=code)
        if not text:
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = transcribe_via_url_assemblyai(telegram_file_url, language=lang)
        if not text:
