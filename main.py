...
def create_progress_message(chat_id, reply_to_message_id, text):
    try:
...
def update_progress_message(chat_id, text):
    try:
        msg_id = progress_messages.get(chat_id)
        if msg_id:
            bot.edit_message_text(text, chat_id, msg_id)
    except:
        pass

def delete_progress_message(chat_id):
    try:
        msg_id = progress_messages.pop(chat_id, None)
        if msg_id:
...
def send_welcome(message):
    if ensure_joined(message):
        welcome_text = (
            ...
            "This bot is not good. For best quality, use @MediaToTextBot any problem report https://t.me/orlaki"
        )
...
def lang_cb(call):
    ...
    prog_id = create_progress_message(chat_id, orig_msg.message_id, "📥 File received\n0%")
    try:
        update_progress_message(chat_id, "🎧 Processing audio…\n10%")
        text = transcribe_file(file_path, language=code, progress_callback=lambda completed, total: transcription_progress(chat_id, orig_msg, completed, total))
        if not text:
            raise ValueError("Empty transcription")
        update_progress_message(chat_id, "✅ Done\n100%")
        delete_progress_message(chat_id)
...
def transcription_progress(chat_id, orig_msg, completed, total):
    try:
        if total <= 0:
            return
        percent = 10 + int(85 * (completed / total))
        if percent > 99:
            percent = 99
        update_progress_message(chat_id, f"✍️ Transcribing…\n{percent}% ({completed}/{total} chunks)")
    except:
        pass
...
def handle_media(message):
    ...
    try:
        ...
        prog_id = create_progress_message(message.chat.id, message.message_id, "📥 File received\n5%")
        if not lang:
            pending_files[message.chat.id] = {"path": dest_path, "message": message}
            kb = build_lang_keyboard("file")
            update_progress_message(message.chat.id, "🎧 Waiting for language selection…\n10%")
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        update_progress_message(message.chat.id, "🎧 Processing audio…\n10%")
        text = transcribe_file(dest_path, language=lang, progress_callback=lambda completed, total: transcription_progress(message.chat.id, message, completed, total))
        if not text:
            raise ValueError("I don't understand this voice 😓")
        update_progress_message(message.chat.id, "✅ Done\n100%")
        delete_progress_message(message.chat.id)
