...
def transcribe_file(file_path, language=None):
    r = sr.Recognizer()
    chunk_seconds = 300
    tmp_dir = tempfile.mkdtemp(prefix="chunks_", dir=DOWNLOADS_DIR)
    out_pattern = os.path.join(tmp_dir, "chunk_%03d.flac")
    ffmpeg_cmd = [
    ...
    try:
        subprocess.run(ffmpeg_cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=REQUEST_TIMEOUT)
    except subprocess.CalledProcessError as e:
        shutil.rmtree(tmp_dir, ignore_errors=True)
        raise RuntimeError(f"ffmpeg error: {e.stderr.decode('utf-8', errors='ignore') if e.stderr else str(e)}")
    except subprocess.TimeoutExpired:
        shutil.rmtree(tmp_dir, ignore_errors=True)
        raise RuntimeError("ffmpeg timed out")
    chunks = sorted([os.path.join(tmp_dir, f) for f in os.listdir(tmp_dir) if f.startswith("chunk_")])
    if not chunks:
        shutil.rmtree(tmp_dir, ignore_errors=True)
