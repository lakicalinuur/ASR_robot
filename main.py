...
def lang_cb(call):
    ...
    try:
        text = transcribe_file(file_path, language=code, progress_chat_id=chat_id)
        if not text:
...
def handle_media(message):
    ...
    try:
        ...
        try:
            progress_msg = bot.send_message(message.chat.id, "Starting download " + render_progress_bar(0,24))
        except:
            progress_msg = None
        download_file_from_telegram(file_info, dest_path, progress_chat_id=message.chat.id, progress_msg_id=(progress_msg.message_id if progress_msg else None))
        ...
        try:
            processing_progress_msg = bot.send_message(message.chat.id, "Processing " + render_progress_bar(0,24))
        except:
            processing_progress_msg = None
