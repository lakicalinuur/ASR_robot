...
def lang_cb(call):
    ...
    processing_progress_msg = None
    try:
        try:
            processing_progress_msg = bot.send_message(chat_id, "Processing " + render_progress_bar(0,24), reply_to_message_id=orig_msg.id)
        except:
            processing_progress_msg = None
        text = transcribe_file(file_path, language=code, progress_chat_id=chat_id, progress_msg_id=(processing_progress_msg.message_id if processing_progress_msg else None))
        if not text:
            raise ValueError("Empty transcription")
        if processing_progress_msg:
            try:
                bot.delete_message(chat_id, processing_progress_msg.message_id)
            except:
                pass
...
def render_progress_bar(percent, width=20):
    try:
...
def download_file_from_telegram(file_info, dest_path, progress_chat_id=None, progress_msg_id=None):
            ...
            for chunk in r.iter_content(chunk_size=65536):
                if chunk:
                    ...
                    if total > 0:
                        ...
                        percent = int(min(99, downloaded // (1024*50)))
                    if percent != last_percent and (now - last_update) > 0.4:
                        last_percent = percent
                        last_update = now
                        if progress_chat_id and progress_msg_id:
                            try:
                                text = f"Downloading file {render_progress_bar(percent, 24)}"
                                bot.edit_message_text(text, progress_chat_id, progress_msg_id)
                            except:
                                pass
        if progress_chat_id and progress_msg_id:
            try:
                bot.edit_message_text(f"Download complete {render_progress_bar(100,24)}", progress_chat_id, progress_msg_id)
            except:
                pass
    return dest_path
...
def transcribe_file(file_path, language=None, progress_chat_id=None, progress_msg_id=None):
    ...
    completed = 0
    last_percent = -1
    if progress_chat_id and not progress_msg_id:
        try:
            pm = bot.send_message(progress_chat_id, "Processing: " + render_progress_bar(0, 24))
            progress_msg_id = pm.message_id
        except:
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            if percent != last_percent:
                ...
                if progress_chat_id and progress_msg_id:
                    try:
                        bot.edit_message_text(f"Processing: {render_progress_bar(percent,24)}", progress_chat_id, progress_msg_id)
                    except:
                        pass
    ...
    if progress_chat_id and progress_msg_id:
        try:
            bot.edit_message_text(f"Processing complete {render_progress_bar(100,24)}", progress_chat_id, progress_msg_id)
        except:
            pass
...
def handle_media(message):
    ...
    try:
        ...
        try:
            download_progress_msg = bot.send_message(message.chat.id, "Starting download " + render_progress_bar(0,24), reply_to_message_id=message.message_id)
        except:
            download_progress_msg = None
        download_file_from_telegram(file_info, dest_path, progress_chat_id=message.chat.id, progress_msg_id=(download_progress_msg.message_id if download_progress_msg else None))
        ...
        if not lang:
            ...
            if download_progress_msg:
                try:
                    bot.delete_message(message.chat.id, download_progress_msg.message_id)
                except:
                    pass
            return
        try:
            processing_progress_msg = bot.send_message(message.chat.id, "Processing " + render_progress_bar(0,24), reply_to_message_id=message.message_id)
        except:
            processing_progress_msg = None
        text = transcribe_file(dest_path, language=lang, progress_chat_id=message.chat.id, progress_msg_id=(processing_progress_msg.message_id if processing_progress_msg else None))
        if not text:
        ...
        try:
            if processing_progress_msg:
                bot.delete_message(message.chat.id, processing_progress_msg.message_id)
        except:
            pass
