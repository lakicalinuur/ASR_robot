...
def lang_cb(call):
    ...
    try:
        text = transcribe_file(file_path, orig_msg, language=code)
        if not text:
...
def transcribe_file(file_path, original_message, language=None):
    ...
    progress_msg = None
    try:
        progress_msg = bot.reply_to(original_message, "⏳ Processing... 0%")
    except:
        pass
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                completed_chunks += 1
                if progress_msg and total_chunks > 0:
                    percent = int((completed_chunks / total_chunks) * 100)
                    try:
                        if percent % 10 == 0 or completed_chunks == total_chunks:
                            bot.edit_message_text(f"⏳ Processing... {percent}%", progress_msg.chat.id, progress_msg.message_id)
                    except:
                        pass
            except Exception as e:
...
def handle_media(message):
    ...
    try:
        ...
        text = transcribe_file(dest_path, message, language=lang)
