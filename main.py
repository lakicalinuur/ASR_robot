...
ASSEMBLYAI_KEYS = os.environ.get("ASSEMBLYAI_KEYS", os.environ.get("ASSEMBLYAI_KEYS", "156773b1e919424f9e065a1313951bc8"))
...
def upload_and_transcribe_assemblyai(file_path=None, remote_url=None, language=None):
    if not assembly_rotator.keys:
        raise RuntimeError("AssemblyAI key(s) not configured")
    def perform_all_steps(key):
        if remote_url:
            transcript_id = create_assemblyai_transcript_with_key(remote_url, key, language_code=language)
        else:
...
def lang_cb(call):
    ...
    try:
        text = upload_and_transcribe_assemblyai(remote_url=remote_url, language=code)
        if not text:
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = upload_and_transcribe_assemblyai(remote_url=telegram_file_url, language=lang)
        if not text:
