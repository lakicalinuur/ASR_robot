...
def _build_progress_text(percent, status, spinner_index=0, bar_length=20):
...
def start_progress_message(chat_id, reply_to):
    try:
        msg = bot.send_message(chat_id, _build_progress_text(0, "Queued", 0), reply_to_message_id=reply_to)
        return msg
    except:
        return None

def edit_progress_message(chat_id, message_id, completed, total, status):
    try:
        percent = int((completed / total) * 100) if total and total > 0 else 100
        spinner_idx = completed
        bot.edit_message_text(_build_progress_text(percent, status, spinner_idx), chat_id, message_id)
    except:
        pass

def finish_and_delete_progress(chat_id, message_id):
    try:
...
def transcribe_file(file_path, language=None, progress_callback=None):
    duration = get_audio_duration(file_path)
    if duration == 0:
        if progress_callback:
            try:
                progress_callback(0, 1, "Invalid duration")
            except:
                pass
        return ""
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        if progress_callback:
            try:
                progress_callback(0, total_chunks, "Transcription started")
            except:
                pass
        for future in as_completed(futures):
            ...
            if progress_callback:
                try:
                    progress_callback(completed, total_chunks, f"Processing {completed}/{total_chunks}")
                except:
                    pass
...
def lang_cb(call):
    ...
    progress_msg = start_progress_message(chat_id, orig_msg.id)
    try:
        def progress_cb(completed, total, status):
            if progress_msg:
                edit_progress_message(chat_id, progress_msg.message_id, completed, total, status)
        text = transcribe_file(file_path, language=code, progress_callback=progress_cb)
        if not text:
        ...
        try:
            if progress_msg:
                finish_and_delete_progress(chat_id, progress_msg.message_id)
        except:
            pass
        try:
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        progress_msg = start_progress_message(message.chat.id, message.id)
        def progress_cb(completed, total, status):
            if progress_msg:
                edit_progress_message(message.chat.id, progress_msg.message_id, completed, total, status)
        text = transcribe_file(dest_path, language=lang, progress_callback=progress_cb)
        if not text:
        ...
        try:
            if progress_msg:
                finish_and_delete_progress(message.chat.id, progress_msg.message_id)
        except:
            pass
