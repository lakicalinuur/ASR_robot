...
def execute_gemini_action(action_callback):
    last_exc = None
    keys = [k.strip() for k in GEMINI_KEYS.split(",") if k.strip()] if isinstance(GEMINI_KEYS, str) else list(GEMINI_KEYS or [])
    pos = 0
    total = len(keys) or 1
    for _ in range(total + 1):
        key = keys[pos] if keys else None
        if not key:
...
def lang_cb(call):
    ...
    if not pending:
        return
    remote_url = pending.get("url")
    orig_msg = pending.get("message")
    bot.send_chat_action(chat_id, 'typing')
    try:
        text = download_and_transcribe(remote_url, language=code)
        if not text:
...
def download_file(url, dest, timeout=REQUEST_TIMEOUT):
...
def convert_to_wav(input_path, output_path):
    subprocess.run(["ffmpeg", "-y", "-i", input_path, "-ar", "48000", "-ac", "1", output_path], check=True)
...
def download_and_transcribe(remote_url=None, file_path=None, language=None):
    tmpdir = tempfile.mkdtemp(dir=DOWNLOADS_DIR)
    try:
        if remote_url:
            local_input = os.path.join(tmpdir, "input")
            download_file(remote_url, local_input)
        elif file_path:
        ...
        try:
            ...
            if local_input.lower().endswith(".wav"):
                wav_path = local_input
            else:
                raise
        text = transcribe_wav(wav_path, language=language)
        return text
    finally:
        try:
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            pending_files[message.chat.id] = {"url": telegram_file_url, "message": message}
            kb = build_lang_keyboard("file")
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = download_and_transcribe(remote_url=telegram_file_url, language=lang)
        if not text:
