...
def transcribe_file(file_path, language=None, chat_id=None, reply_id=None):
    ...
    try:
        if chat_id is not None and reply_id is not None:
            try:
                progress_msg = bot.send_message(chat_id, "Processing: 0% ⏳", reply_to_message_id=reply_id)
            except:
                progress_msg = None
            ...
            for future in as_completed(futures):
                ...
                try:
                    if progress_msg:
                        ...
                        if percent > 100:
                            percent = 100
                        bot.edit_message_text(f"Processing: {percent}% ⏳", chat_id, progress_msg.message_id)
                except:
                    pass
